...
## Battle pose sheet (2 × 3: idle, attack, technique / hurt, KO, victory) when its art
## exists; otherwise the hero keeps the reference sheet's battle stance.
func _battle_poses(id: String, frame: Vector2i) -> Dictionary:
	var path := "res://assets/sprites/characters/%s/chr_%s_battle.png" % [id, id]
	if not ResourceLoader.exists(path):
		return {}
	return {"battle_sheet": load(path), "frame_size": frame, "idle_frame": 0, "attack_frame": 1, "cast_frame": 2,
		"hurt_frame": 3, "ko_frame": 4, "victory_frame": 5, "flip_h": false}
...
func _build_characters() -> void:
	_hero("kael", {"starting_equipment": [_items["gear_blade"], _items["leather_coat"]] as Array[ItemData],
		...
		"portrait": load("res://assets/portraits/por_kael_neutral.png")}.merged(_battle_poses("kael", Vector2i(94, 82)), true))
	...
	_hero("eco", {"starting_equipment": [_items["leather_coat"]] as Array[ItemData],
		...
		"status_immunities": [S.POISON, S.SLEEP] as Array[int]}.merged(_battle_poses("eco", Vector2i(73, 67)), true))
...
const MUSIC_EMPIRE := "res://assets/audio/music/bgm_empire_b.mp3"
const MUSIC_FAREWELL := "res://assets/audio/music/bgm_farewell_b.mp3"
...
func _build_prologue_story() -> void:
	...
	_cutscene("p8_voss", "voss_fought", [
		...
		_step(ST.SFX, {"id": &"explosion_small"}),
